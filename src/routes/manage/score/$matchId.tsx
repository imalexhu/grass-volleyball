...
import { ArrowLeft, Undo2, Video, Check, Save, Youtube, Zap, Radio, Trophy, History, Star, UploadCloud, Loader2 } from "lucide-react";
...
const handleHighlight = async () => {
  ...
  const reversedIdx = [...events].reverse().findIndex((e: MatchEvent) => e.type === "point");
  if (reversedIdx === -1) {
    toast.error("No points to highlight");
    return;
  }
  const lastPointIdx = events.length - 1 - reversedIdx;
    ...
    {match.status === "scheduled" && (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/80 backdrop-blur-md px-4">
        <div className="max-w-md w-full bg-card border border-border rounded-3xl p-8 shadow-2xl text-center">
          ...
          <Link to="/manage" className="block mt-6 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] hover:text-foreground transition-colors">
            Exit to Dashboard
          </Link>
        </div>
      </div>
